...
public class MapDriverActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        nMapFragment = (SupportMapFragment) getSupportFragmentManager().findFragmentById(R.id.map_driver);

        mFusedLocation = LocationServices.getFusedLocationProviderClient(this);

        nMapFragment.getMapAsync(this);
        mButtonConect = findViewById(R.id.btn_Conect);
    ...
    protected void onDestroy() {
        super.onDestroy();
        if(mListener != null){
            mGeofireProvier.isDriverWorking(mAhutProvider.getId()).removeEventListener(mListener);
        }else{
            System.out.println("MapDriverAct: onDestroy() mListener es null");
        }
    }

    private void isDriverWorking() {
        mListener = mGeofireProvier.isDriverWorking(mAhutProvider.getId()).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                Log.i("MapDriverAc", "isDriverWorking(): snapsot: "+ snapshot);
                System.out.println("MapDriverAc mListener: " + mListener);
                if(snapshot.exists()){
                    //dejar de escuchar la ubicacion en tiempo real del mapDriverActivity
                    disconnect();
                }else{
                    System.out.println("MapDriverAct Error: \nlinea 157: \nel snapshot no exist" + snapshot);
                }
            }

            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                System.out.println("Error: MapDriverAc isDriverWorking " + error);
            }
        });
    }
    ...
    private void disconnect(){
        if(mFusedLocation != null){
            mButtonConect.setText("Conectar");
            isConect = false;
    ...
    private void starLocation(){
        if(Build.VERSION.SDK_INT>= Build.VERSION_CODES.M){
            if (ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED){
                if(GPSActivte()){
                    mButtonConect.setText("Desconectar");
                    isConect = true;
                    mFusedLocation.requestLocationUpdates(mLocationRequest, mLocationCallback, Looper.myLooper());
                }else{
                    ShowAlerDialogNoGPS();
